package br.com.caju.domain.{{cookiecutter.resource_name_lower}}.command

import br.com.caju.domain.{{cookiecutter.resource_name_lower}}.model.{{cookiecutter.resource_name_camel}}
import java.util.UUID

data class Update{{cookiecutter.resource_name_camel}}Command(
    val id: UUID,{% if cookiecutter.resource_name == 'Article' %}
    val title: String,
    val content: String,
    val status: String
{% else %}    // TODO: Add your domain-specific fields here
    // Examples: val name: String, val email: String, etc.
{% endif %})

fun Update{{cookiecutter.resource_name_camel}}Command.toModel() = {{cookiecutter.resource_name_camel}}(
    id = id,{% if cookiecutter.resource_name == 'Article' %}
    title = title,
    content = content,
    status = status,
{% else %}    // TODO: Map your command fields to model fields here
{% endif %}    createdAt = null, // Will be set by persistence layer
    updatedAt = null  // Will be set by persistence layer
)
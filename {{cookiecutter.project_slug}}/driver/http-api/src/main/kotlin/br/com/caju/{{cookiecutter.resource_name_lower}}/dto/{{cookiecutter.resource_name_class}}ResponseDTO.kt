...
import br.com.caju.shared.dto.map
{% endif %}
import java.time.LocalDateTime
import java.util.UUID

data class {{cookiecutter.resource_name_class}}ResponseDTO(
    val id: UUID,
...
fun {{cookiecutter.resource_name_class}}.toDTO() = {{cookiecutter.resource_name_class}}ResponseDTO(
    id = id,
{% if cookiecutter.resource_name == 'Article' %}
    title = title,
    content = content,
    status = status,
{% else %}
    // TODO: Map your model fields to DTO fields here
{% endif %}
    createdAt = createdAt,
    updatedAt = updatedAt
)
...
fun PaginatedModel<{{cookiecutter.resource_name_class}}>.toResponseDTO(
    ...
    this.toPaginatedResponseDTO(pagination).map { it.toDTO() }
{% endif %}
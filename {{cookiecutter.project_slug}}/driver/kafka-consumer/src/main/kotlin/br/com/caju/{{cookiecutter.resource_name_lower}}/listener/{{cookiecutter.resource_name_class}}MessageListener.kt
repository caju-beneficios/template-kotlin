...
@Service
class {{cookiecutter.resource_name_class}}MessageListener {

    @Trace(
        resourceName = TP_{{cookiecutter.resource_name_constant}}_PRODUCER_TOPIC,
    ...
    @KafkaListener(
        ...
        topics = [TP_{{cookiecutter.resource_name_constant}}_PRODUCER_TOPIC],
        properties = [OFFSET_RESET_EARLIEST],
    )
    fun receive(@Payload {{cookiecutter.resource_name_lower}}MessageListenerDTO: {{cookiecutter.resource_name_class}}MessageListenerDTO) {
{% if cookiecutter.resource_name == 'Article' %}
        logger.info("{{cookiecutter.resource_name_class}} Event consumed=${{cookiecutter.resource_name_lower}}MessageListenerDTO")
...
{% endif %}
    ...
    companion object {
        private const val TP_{{cookiecutter.resource_name_constant}}_PRODUCER_TOPIC = "tp_{{cookiecutter.resource_name_lower}}_event_create"
